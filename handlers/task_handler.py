...
def display_name(user) -> str:
    """Return @username if set, otherwise full_name."""
    return f"@{user.username}" if getattr(user, "username", None) else user.full_name


def back_to_menu() -> InlineKeyboardMarkup:
    """Single back button used on all info screens."""
...
async def safe_edit(callback: CallbackQuery, text: str, reply_markup=None, parse_mode: str = "HTML"):
    """Edit current message in place. Falls back to answer() if message is too old or unchanged."""
    try:
...
def task_slots_keyboard(tasks: list, skip_available: bool = True) -> InlineKeyboardMarkup:
    ...
    bottom = []
    if skip_available:
        bottom.append(InlineKeyboardButton(text="⏭️ Skip Today (1×/week)", callback_data="grp_skip"))
    bottom.append(InlineKeyboardButton(text="✔️ Done Setting Tasks", callback_data="grp_finalize_tasks"))
...
async def grp_finalize_tasks(callback: CallbackQuery, user, session, state: FSMContext):
    ...
    if not tasks:
        await callback.answer("⚠️ Add at least one task before finalising!", show_alert=True)
        return
...
async def grp_submit(callback: CallbackQuery, user, session):
    ...
    if len(tasks) >= MAX_SLOTS:
        await callback.answer(
            "You've filled all 6 task slots today! Use 'Update Task' to mark them done.",
            show_alert=True,
        )
        return
...
# Must use message.answer() — responding to user text, cannot edit_text.
# ─────────────────────────────────────
...
async def grp_mark_done(callback: CallbackQuery, user, session):
    ...
    await safe_edit(callback, "\n".join(msg_lines), reply_markup=update_task_keyboard(fresh_tasks))
    await callback.answer()

    # Public announcement when ALL tasks completed
    if all_done:
        name = display_name(user)
        await callback.bot.send_message(
            group_id,
...
# Skip logic completely untouched — only display changed.
# ─────────────────────────────────────
